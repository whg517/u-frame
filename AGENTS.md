...
├── application/         # 用例和事务编排
├── domain/              # 纯领域规则
├── infrastructure/      # SQLite、Excel、备份等 adapter
├── dto/                 # IPC DTO
├── state.rs
└── lib.rs
```
...
- Repository/Adapter 负责 SQLite、Excel、CSV、备份和系统路径。
...
- 不得在数据库写入过程中直接复制 SQLite 主文件。
- 使用一致性快照，并包含版本和摘要信息。
- 恢复前校验格式、摘要、数据库完整性和兼容版本。
- 恢复前自动创建当前数据的安全备份。
- 恢复失败不得留下部分恢复状态。

## 10. 安全约束

- React WebView、Excel、备份文件和用户提供的路径均视为不可信输入。
...
- 数据迁移必须向前演进；禁止用删除用户数据库作为升级方案。
- 未经用户明确要求，不提交、推送、发布、签名或公证产物。
- 不把构建产物、数据库、日志、导入文件或本地备份提交到 Git。
