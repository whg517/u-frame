...
| 外观、主题色、语言、默认画布缩放、界面密度、字号和默认启动页面偏好 | React Context + 带版本的 `localStorage`，启动渲染前恢复 |
...
- 跨页操作通过 `returnTo` 保留发起页面的 URL 筛选和选中上下文；共享导航工具只接受以 `/` 开头且不以 `//` 开头的应用内路径。
- 已知父级或目标对象使用 `roomId`、`areaId` 或 `rackId` URL 参数预选表单；后端仍校验关联对象是否存在且可用。
- 详情页把“新增下级对象”、“选择已有设备”、“移动”和“下架”作为与当前对象相关的后续动作，而不要求用户回到一级导航重新定位。
...
- 机房和区域筛选使用受控 Popover，页面统一持有当前打开项，以保证两者不重叠，并获得 Esc、点击外部和焦点管理行为。

### 5.6 台账与放置交互

- 设备搜索和类型、状态、上架状态筛选写入 URL，组件根据 Query 缓存结果做纯前端组合筛选；本地 MVP 数据规模达到性能阈值后再下沉到 Rust 查询。
...
- 国际化资源集中在 `shared/i18n`；简体中文作为稳定消息键和默认文案，英语资源提供完整映射，插值仅用于数字、实体名称和 U 位等动态值。
- 业务实体名称、编码、厂商、型号、IP、SN 和用户输入内容保持原值，不进行机器翻译。
...
- 默认启动页面提供机柜一览、位置管理、机柜管理和设备资产四个一级入口，默认值为机柜一览；设置选择立即保存，但只在下次应用启动时解析。
- 应用入口在 React 首屏渲染前仅对无参数、无 hash 的根路径执行 `history.replaceState`；非根路径、详情深链和带筛选上下文的根路径保持不变。
- Browser Router 延迟到 `AppRouter` 首次渲染时创建，确保它读取的是启动偏好解析后的路径；启动后用户再次访问 `/` 不触发重复重定向。
