...
2026-09-04 根据实际桌面窗口复查，修正应用壳层和机柜一览页的高度约束：

- 应用壳、固定侧栏和主工作区统一约束为 `100dvh`，页面内部独立滚动。
- 机柜一览收敛为 64px 单行工具栏，画布填满全部剩余空间。
- 空状态直接居中于画布，不再使用固定高度的虚线卡片。
- 机柜组在可用画布内垂直居中，仍按 U1 基线对齐。
- 1280×720 浏览器测量确认侧栏和主工作区高度均为 720px，画布区域为剩余 656px；实际 Tauri 窗口复核空状态、多机柜和设备详情三种状态。
